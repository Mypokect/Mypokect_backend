...
class TaxController extends Controller
{
    ...
    public function getData(Request $request): JsonResponse
    {
        try {
            ...
            $hasRentTypeCol = SchemaCache::hasColumn('movements', 'rent_type');
            ...
            if (SchemaCache::hasColumn('movements', 'is_business_expense')) {
    ...
    public function recalculate(Request $request): JsonResponse
    {
        try {
            ...
            if (! empty($data['tipo_contribuyente'])) {
                ...
                if ($bolsaMax == 0 || $bolsaMax == $laboral) {
                    $tipo = 'empleado';
                } elseif ($bolsaMax == $honorarios) {
                    $tipo = 'independiente';
                } elseif ($bolsaMax == $comercial) {
                    $tipo = 'comerciante';
                } else {
                    $tipo = 'rentista';
                }
            ...
            if ($tieneIngresosNoLaborales && SchemaCache::hasColumn('movements', 'is_business_expense')) {
            ...
            $hasRentTypeColR = SchemaCache::hasColumn('movements', 'rent_type');
            $keywordsMapR = [
                'laboral' => ['sueldo', 'nómina', 'nomina', 'salario', 'quincena'],
                'honorarios' => ['honorarios', 'servicios', 'freelance'],
                'capital' => ['arriendo', 'alquiler', 'intereses', 'dividendos'],
            ...
            if (! empty($asignacionesGastos) && $tieneBolsas && SchemaCache::hasColumn('movements', 'is_business_expense')) {
    ...
    public function getCedularMovements(Request $request): JsonResponse
    {
        try {
            ...
            $hasRentType = SchemaCache::hasColumn('movements', 'rent_type');
    ...
    public function moveCedularMovement(Request $request): JsonResponse
    {
        try {
            ...
            if (SchemaCache::hasColumn('movements', 'rent_type')) {
    ...
    private function applyArt241(float $baseUvt): float
    {
        ...
        if ($baseUvt <= 1700) {
            return ($baseUvt - 1090) * 0.19;
        }
        if ($baseUvt <= 3400) {
            return ($baseUvt - 1700) * 0.28 + 116;
        }
        if ($baseUvt <= 4100) {
            return ($baseUvt - 3400) * 0.33 + 592;
        }
        if ($baseUvt <= 8670) {
            return ($baseUvt - 4100) * 0.36 + 823;
        }
        if ($baseUvt <= 18970) {
            return ($baseUvt - 8670) * 0.38 + 2469;
        }
        if ($baseUvt <= 31000) {
            return ($baseUvt - 18970) * 0.40 + 6383;
        }
    ...
    private function generateSummaryMessage(array $alerts): string
    {
        ...
        if ($exceeded > 0) {
            return "⚠️ Superaste $exceeded tope(s) fiscal(es). Consulta a tu contador.";
        }
        if ($warning > 0) {
            return "🟡 Te acercas a $warning límite(s). Monitorea tus ingresos.";
        }
