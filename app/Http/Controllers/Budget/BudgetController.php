...
class BudgetController extends Controller
{
     ...
     */
    /**
     * Reclassify a movement to a different category within the SAME budget.
     *
     * Changes the movement's tag_id to the first linked_tag of the target category.
     * Validates that the target category belongs to this budget (prevents cross-budget leaks).
     * Returns updated category execution data so the client can refresh bars immediately.
     *
     * @bodyParam movement_id int required The movement ID to reclassify. Example: 42
     * @bodyParam new_category_name string required Target category name within this budget. Example: Transporte
     */
    public function reclassifyMovement(Request $request, Budget $budget): JsonResponse
    {
        try {
            ...
            $movementId  = (int) $request->input('movement_id');
            $newCatName  = $request->input('new_category_name');

            // ── Verificar que el movimiento pertenece al usuario ───────────────
            $movement = Movement::where('user_id', Auth::id())
                ->where('id', $movementId)
                ->first();
            ...
            // ── Verificar que la categoría destino pertenece a ESTE presupuesto ─
            // Previene que un movimiento se escape a categorías de otros presupuestos.
            $budget->load('categories');
            $targetCategory = $budget->categories->first(fn ($c) => $c->name === $newCatName);

            if (! $targetCategory) {
                return $this->validationErrorResponse(
                    null,
                    "La categoría '$newCatName' no pertenece a este presupuesto."
                );
            }

            // ── Resolver el primer tag vinculado a la categoría destino ──────
            $targetTagNames = $this->extractTagNames($targetCategory->linked_tags ?? []);

            if (empty($targetTagNames)) {
                // La categoría no tiene tags configurados: crear/obtener uno con el nombre de la categoría
                $targetTagName = $newCatName;
            } else {
                $targetTagName = $targetTagNames[0];
            }
            ...
            // ── Actualizar el tag_id del movimiento ───────────────────────────
            $movement->update(['tag_id' => $tag->id]);

            Log::info('Movement reclassified within budget', [
                'movement_id' => $movementId,
                'budget_id'   => $budget->id,
                'new_category' => $newCatName,
                'new_tag'     => $targetTagName,
                'user_id'     => Auth::id(),
            ]);

            // ── Recalcular ejecución por categoría y devolver ─────────────────
            // El cliente actualiza las barras de progreso en tiempo real sin
            // necesitar un segundo request a /spending.
            $budget->load('categories');
            $updatedCategories = $this->enrichCategoriesWithExecution($budget);
